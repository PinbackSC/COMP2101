...
#Added a function that is called with the resetTime trap below

function resetTime {
  echo ""
  echo ""
  echo "You are not aloud to skip the counter!"
  sleepCount=$numberOfSleeps
}

#add a trap to call the resetTime function when the script recieves an INT signal (CTRL + C)
trap resetTime SIGINT

#======================
#======================
#======================

#Added a function to run when the script recievs a QUIT signal
...
#Added a trap to call the userQuit function when the script revieves a QUIT signal
#this will run automatically at the end of the timer as well, because its technically a QUIT.
